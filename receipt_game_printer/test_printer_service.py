...
from printer_service import NF_LOGO_HEIGHT, NF_LOGO_WIDTH, PrinterService, _build_logo_pixels
...
class PrinterServiceLogoTests(unittest.TestCase):
    def test_nf_logo_is_a_small_nonempty_monogram_without_f_bottom_arm(self):
        ...
        self.assertEqual((NF_LOGO_WIDTH, NF_LOGO_HEIGHT), (58, 18))
        ...
        # The final rows stay blank, preventing the old underline artifact.
        self.assertFalse(any(pixels[-1]))
        self.assertFalse(any(pixels[-2]))

        # Below the F's middle arm there must be no horizontal run (an E-like
        # lower arm); only the shared slanted N/F stem may remain.
        for row in pixels[9:]:
            f_pixels = [x for x in range(24, NF_LOGO_WIDTH) if row[x]]
            self.assertLessEqual(len(f_pixels), 1)
