...
class PrinterServiceLogoTests(unittest.TestCase):
    def test_nf_logo_is_a_small_nonempty_monogram_without_f_bottom_arm(self):
        ...
        self.assertGreaterEqual(NF_LOGO_WIDTH, 42)
        self.assertLessEqual(NF_LOGO_WIDTH, 58)
        self.assertGreaterEqual(NF_LOGO_HEIGHT, 14)
        self.assertLessEqual(NF_LOGO_HEIGHT, 18)
        ...
        # lower arm); only the shared N/F transition may remain.
        for row in pixels[9:]:
            self.assertFalse(any(row[24:]))
