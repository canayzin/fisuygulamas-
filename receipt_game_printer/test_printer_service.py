...
class PrinterServiceLogoTests(unittest.TestCase):
    def test_nf_logo_is_a_small_nonempty_monogram_without_f_bottom_arm(self):
        ...
        self.assertLess(NF_LOGO_WIDTH, 64)
        self.assertLess(NF_LOGO_HEIGHT, 22)
        ...
        self.assertGreater(sum(map(sum, pixels)), 0)
        self.assertNotEqual(NF_LOGO_ESC_STAR, b"NF")

        # The matrix is the natural black-pixel bounds, not a padded canvas.
        self.assertTrue(any(pixels[0]))
        self.assertTrue(any(pixels[-1]))
        self.assertTrue(any(row[0] for row in pixels))
        self.assertTrue(any(row[-1] for row in pixels))

        # The N valley is at x=22 and its shared F junction at x=30.  The
        # final rising diagonal is deliberately shorter than the old 13px
        # span, which is the key compacting change from the physical proof.
        self.assertTrue(pixels[13][22])
        self.assertTrue(pixels[4][30])
        self.assertLess(30 - 22, 13)
        ...
        for row in pixels[10:]:
            self.assertFalse(any(row[31:]))

        # Adjacent occupied rows share at least one column, keeping the thin
        # diagonals physically connected without imposing artificial weight.
        ...
        for upper, lower in zip(occupied_rows, occupied_rows[1:]):
            if upper and lower:
                self.assertGreaterEqual(len(upper & lower), 1)
