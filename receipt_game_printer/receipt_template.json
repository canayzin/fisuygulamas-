...
def _address_lines(
    ...
    source = [
        line
        for line in (data.address_line1, data.address_line2)
        if line
...
def _footer_logo_lines(
    ...
    if template.use_bitmap_nf_logo:
        logo_line = "  ".join(
            part for part in ("[NF LOGO]", logo_code) if part
        ).strip()
        return [center(logo_line, width)]

    logo = (template.footer_logo_text or "NF").strip()
    logo_line = "  ".join(
        part for part in (logo, logo_code) if part
    ).strip()
...
def build_receipt_text(
    ...
    separator = separator_char * min(width, max(width - 2, 1))
    ...
    for header in header_rows:
        ...
        if template.center_firm_name:
            rows.append(center(header, width))
        else:
            rows.append(_fit_line(header, width))
    ...
    eku_no = (
        data.eku_no
        if template.use_firm_eku_z and data.eku_no
        else template.eku_no
    )
    z_no = (
        data.z_no
        if template.use_firm_eku_z and data.z_no
        else template.z_no
    )
