...
def _build_logo_pixels() -> list[list[int]]:
    ...
    # Compact, single-stroke italic monogram.  The N's rising right stroke is
    # deliberately shared with the F stem, matching the linked mark on the
    # reference receipt instead of rendering two ordinary text glyphs.
    _thick_line(pixels, 2, 15, 7, 2, 0)    # N: short rising left stroke
    _thick_line(pixels, 7, 2, 20, 15, 0)   # N: descending diagonal
    _thick_line(pixels, 20, 15, 27, 2, 0)  # N right / F shared stem
    _thick_line(pixels, 27, 2, 55, 2, 0)   # F: slender upper arm
    _thick_line(pixels, 24, 8, 46, 8, 0)   # F: slender middle arm
    return pixels
